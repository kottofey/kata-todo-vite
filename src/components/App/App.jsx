...
export default function App() {
  ...
  const updateTimer = (itemId, minutes, seconds, timerId) => {
  ...
  const timerTick = (itemId, minutes, seconds, timerId) => {
    ...
    if (newMinutes === 0 && newSeconds === 0) {
      updateTimer(itemId, newMinutes, newSeconds, timerId);
      return;
    }

    const newTimerId = setTimeout(() => {
      timerTick(itemId, newMinutes, newSeconds, newTimerId);
    }, 1000);
    updateTimer(itemId, newMinutes, newSeconds, newTimerId);
  };

  const onTimerStart = (itemId) => {
    const idx = getItemIndex(todoItems, itemId);
  ...
  const onTimerPause = (itemId) => {
    ...
    updateTimer(itemId, minutes, seconds, null);
  };
